...
"""
=============================================================================
Day 3 — CS LoRA SFT (CodeLlama-7B-Instruct-hf)
=============================================================================
Fine-tune CodeLlama-7B-Instruct with LoRA on the CS SFT dataset from Day 1.

Designed to run IN PARALLEL with Day 2 on a separate Colab account.
...
Differences from Day 2 (Math):
  - Base model: CodeLlama-7B-Instruct (vs DeepSeek-Math-7B)
  - Batch size: 1 with grad_accum=8 (CodeLlama is slightly larger in memory)
  - Data: cs_train.jsonl / cs_val.jsonl
  - Chat template: Llama [INST] format
  - Validation prompts: CS-specific (code, algorithms)

Run on Google Colab (T4 GPU — Account #2):
  1. Upload cs_train.jsonl & cs_val.jsonl to this account's Drive
  2. Upload this script to Colab
  3. Run: !python day3_cs_sft.py
...
class TrainingConfig:
    ...
    # ── Paths (Colab Account #2) ──
    # NOTE: If using a different Google account, adjust these paths.
    # Upload cs_train.jsonl and cs_val.jsonl to your Drive first.
    data_dir: str = "/content/drive/MyDrive/Colab_text_modelling_dp_01/data/processed/sft_data"
    output_dir: str = "/content/drive/MyDrive/Colab_text_modelling_dp_01/lora_cs"
    logging_dir: str = "/content/drive/MyDrive/Colab_text_modelling_dp_01/lora_cs/logs"

    # ── Model ──
    base_model: str = "codellama/CodeLlama-7b-Instruct-hf"
    trust_remote_code: bool = False   # CodeLlama doesn't need trust_remote_code
    ...
    # CodeLlama uses slightly more VRAM than DeepSeek-Math in some configs,
    # so we use batch_size=1 with grad_accum=8 for the same effective batch of 8.
    num_epochs: int = 3
...
log = logging.getLogger("day3_cs_sft")
...
def load_sft_data(config: TrainingConfig):
    """Load the CS JSONL SFT data produced by Day 1."""
    from datasets import Dataset

    train_path = os.path.join(config.data_dir, "cs_train.jsonl")
    val_path = os.path.join(config.data_dir, "cs_val.jsonl")
...
def load_model_and_tokenizer(config: TrainingConfig):
    """Load CodeLlama-7B-Instruct with 4-bit quantization and attach LoRA."""
    ...
    # CodeLlama typically has a pad token, but ensure it
...
def format_chat_to_text(example, tokenizer):
    ...
    CodeLlama-Instruct uses the Llama 2 [INST] format:
      <s>[INST] <<SYS>>\n{system}\n<</SYS>>\n\n{user} [/INST] {assistant}</s>

    We try the tokenizer's built-in chat template first, then fall back
    to the canonical Llama 2 format.
    """
    messages = example["messages"]

    # Try the tokenizer's native chat template
    try:
    ...
    class MilestoneCheckpointCallback(TrainerCallback):
        ...
        def on_step_end(self, args, state, control, **kwargs):
            ...
            if current_step in self.milestone_steps and current_step not in self.saved:
                ...
                self._pending_copy = (current_step, name)
            return control
...
def sanity_check(model, tokenizer, config: TrainingConfig):
    ...
    test_prompt = (
        ...
        "<</SYS>>\n\n"
        "[TRANSCRIPT] A linked list is a data structure where each node "
        "contains a data field and a pointer to the next node.\n"
        "[VISUAL] Computer_Code [CODE] [/INST] "
    )
...
def validate_adapter(model, tokenizer, config: TrainingConfig):
    """Generate a few sample outputs to verify the CS adapter works."""
    ...
    test_prompts = [
        {
            "transcript": (
                ...
                "is a separate object called a node. Each node contains two "
                "fields: data and a pointer called next that refers to the "
                "next node in the sequence."
            ),
            "visual": "Computer_Code",
        },
        {
            "transcript": (
                "The binary search algorithm works by repeatedly dividing the "
                "search interval in half. We compare the target value with the "
                "middle element. If the target is less, we search the left half. "
        ...
        {
            "transcript": (
                "In C++, inheritance allows a class to inherit members from "
                "another class. The derived class uses a colon followed by the "
                "access specifier and base class name. For example, class Dog : "
                "public Animal defines Dog as inheriting from Animal."
            ),
            "visual": "Computer_Code",
        },
    ]
    ...
    for i, prompt in enumerate(test_prompts):
        messages = [
            {"role": "system", "content": "You are a computer science lecture summarizer. Summarize the lecture segment preserving all code references, function names, variable names, and technical accuracy. Use proper code formatting for technical terms."},
            {"role": "user", "content": f"[TRANSCRIPT] {prompt['transcript']}\n[VISUAL] {prompt['visual']} [CODE]"},
        ]

        # Format using the Llama [INST] template
        try:
...
def main():
    log.info("=" * 60)
    log.info(" DAY 3 — CS LoRA SFT (CodeLlama-7B-Instruct)")
    log.info("=" * 60)
    ...
    log.info("  3. Download lora_cs/final/ adapter for Day 4 evaluation")
    log.info("  4. Download both adapters to Account #3 for Day 4")
    log.info("=" * 60)
